...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            ...
            # another solution:
            # left_neg, left_pos = np.sum(left_split == 0), np.sum(left_split == 1)
            # right_neg, right_pos = np.sum(right_split == 0), np.sum(right_split == 1)
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        feature_subset = (
            feature_subset if feature_subset is not None else int(np.sqrt(X.shape[1]) + 1)
        )
